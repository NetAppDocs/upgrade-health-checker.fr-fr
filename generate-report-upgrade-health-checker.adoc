...
** Connexion au point de terminaison de télémétrie (+https://support.netapp.com/+ et +https://api.activeiq.netapp.com+) via HTTPS
** Connectivité aux points de terminaison de mise à jour automatique (+https://api.uhc.netapp.com+, +https://gql.aiq.netapp.com+, et +https://api.activeiq.netapp.com+) via HTTPS (443)
...
. Liste des fichiers de rapport générés :
+
`ls -lrt`

. Ouvrez le fichier de rapport (nommé  `uhc_<cluster-name>_<YYYYMMDDHHMMSS>`) dans un navigateur web, Microsoft Excel ou un visualiseur JSON pour consulter le rapport. Si vous l'exécutez sur un hôte distant, téléchargez d'abord le fichier de rapport sur une machine où vous pourrez le consulter dans un navigateur web, dans Excel ou dans un visualiseur JSON.
+
...
.Et ensuite
Après avoir généré votre plan de mise à niveau et votre rapport de cluster, vous pouvez consulter le link:dashboard-upgrade-health-checker.html["Mise à niveau du tableau de bord Health Checker"] pour obtenir des détails sur l'état de santé des clusters évalués lors de l'exécution.
